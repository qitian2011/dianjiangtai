...
微信小程序端（本目录）
   ...
   ▼
云函数 cloudfunctions/proxy      ← 微信云开发；持有 PIN 环境变量，转发时自动注入
   │  HTTPS 转发
   ▼
云端服务 app/worker.js           ← Cloudflare Workers + Durable Object（qitian.dpdns.org）
```

- **小程序端不感知、不传输云端访问密码**：PIN 由 proxy 云函数环境变量持有并注入（服务端覆盖，客户端无法伪造）
- **班级密码**是运行时数据（教师在本班设置），与云端 PIN 是两回事
...
4. **配 PIN**：云开发控制台 → 云函数 → proxy → 配置 → 新增环境变量
   - `PIN` = 与云端 worker 一致的访问口令（`wrangler secret put PIN` 设置的同一个值）
   - 建议把「执行超时」调到 10~15 秒（跨境转发需要）
5. **连接云端**：设置页「连接与班级管理」→ 默认 `https://qitian.dpdns.org`；如需自建云端请改 `cloudfunctions/proxy/index.js` 的 `BASE_URL` 并部署云端 `app/worker.js`
6. **编译运行**：进入后自动连接示例班，即可点名/传呼

> 没有自己的云端服务时无法使用——本小程序是**控制端**，必须有配套云端/大屏端在线。

## 开源说明

MIT License。真实学生名单属于隐私数据，不在本仓库；云端部署所需 PIN 一律通过平台环境变量注入，代码中无任何明文口令。详见仓库根目录 LICENSE 与 README。
